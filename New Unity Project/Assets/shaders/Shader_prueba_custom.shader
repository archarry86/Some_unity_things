	  ...
	  _Shininess("Shininess", Float) = 10
		...
		SubShader{
		  ...
		  vertexOutput vert(vertexInput v) {
			...
			half4 _posWorld = mul(unity_ObjectToWorld, v.normal);

			half3 worldN = mul((float3x3)unity_ObjectToWorld, v.normal);// mul((float3x3)unity_ObjectToWorld, SCALED_NORMAL);
			half3 shlight = ShadeSH9(_posWorld);
			o.posWorld = float4(shlight, _posWorld[3]);
